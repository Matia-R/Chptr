...
export async function publishDocument(
  ...
  const blocks = parsePublishedBlocks(blocksParsed);
  if (!publishedBlocksHaveContent(blocks)) {
    throw new TRPCError({
      code: "BAD_REQUEST",
  ...
  const bodyHtml = await publishedBlocksToHtml(blocks);
  if (bodyHtml.length > 3_000_000) {
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: "Published document is too large.",
    });
  }
...
export async function syncPublicationOwnerUsernameForCreator(
  ...
  const existing = (existingResult.data ??
    []) as PublicationSlugWithDocumentRow[];
  ...
  for (const row of stale) {
    ...
    if (blockingRedirect && blockingRedirect.document_id !== row.document_id) {
      throw new TRPCError({
        code: "CONFLICT",
        message:
